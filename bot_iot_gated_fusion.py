"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   BoT-IoT Intrusion Detection System — GATED FUSION LOCAL VERSION          ║
║   Pipeline: Decision Tree + XGBoost + Gated Image-Tabular Fusion + RF/LGBM  ║
║             + Late Fusion | PCA + LDA + train-only WCGAN-GP                 ║
║   Dataset : BoT-IoT 5 percent CSV shards (category target)                  ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
HOW TO RUN:
    ...
    3. python bot_iot_gated_fusion.py
    4. Results saved to ./outputs_bot_iot_gated_fusion/
...
# ─────────────────────────────────────────────────────────────────────────────
DATASET_FOLDER = "./data"          # Folder containing BoT-IoT CSV shards
DATASET_FILE   = "./data/UNSW_2018_IoT_Botnet_Full5pc_*.csv"
OUTPUT_DIR     = "./outputs_bot_iot_gated_fusion"  # Separate experiment outputs
ROW_LIMIT      = 300_000           # Fast local stratified sample across shards
CHUNKSIZE      = 200_000           # Stream large shards instead of loading all rows
SHARD_SAMPLE_FRAC = 0.09           # Approximate pre-sample before final ROW_LIMIT
...
def _notify_on_exception(exc_type, exc_value, exc_traceback):
    notify_user("BoT-IoT IDS Failed", str(exc_value)[:120])
...
print("=" * 70)
print("  BoT-IoT IDS — Local Run (Gated Fusion + Train-only WCGAN-GP)")
print("=" * 70)
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 1 — Load BoT-IoT Dataset
# ══════════════════════════════════════════════════════════════════════════════
print("─" * 70)
print("STEP 1: Loading BoT-IoT Dataset")
print("─" * 70)

t0_load = time.time()
csv_paths = sorted(Path(".").glob(DATASET_FILE))
if not csv_paths and DATASET_FOLDER.exists():
    csv_paths = sorted(DATASET_FOLDER.glob("UNSW_2018_IoT_Botnet_Full5pc_*.csv"))

if not csv_paths:
    print(f"❌ BoT-IoT dataset shards not found. Expected pattern: {DATASET_FILE}")
    sys.exit(1)

usecols = [
    'flgs', 'flgs_number', 'proto', 'proto_number',
    'pkts', 'bytes', 'state', 'state_number', 'dur', 'mean', 'stddev',
    'sum', 'min', 'max', 'spkts', 'dpkts', 'sbytes', 'dbytes',
    'rate', 'srate', 'drate', 'TnBPSrcIP', 'TnBPDstIP',
...
parts = []
for shard_id, csv_path in enumerate(csv_paths):
    shard_parts = []
    for chunk_id, chunk in enumerate(pd.read_csv(
        csv_path, usecols=usecols, low_memory=False, chunksize=CHUNKSIZE
    )):
        part = chunk.sample(frac=SHARD_SAMPLE_FRAC, random_state=SEED + shard_id + chunk_id)
        shard_parts.append(part)
    shard_df = pd.concat(shard_parts, ignore_index=True)
    parts.append(shard_df)
    print(f"Sampled shard: {csv_path.name} — shape: {shard_df.shape}")
    del shard_parts, shard_df; gc.collect()

dataset = pd.concat(parts, ignore_index=True)
del parts; gc.collect()
if ROW_LIMIT and len(dataset) > ROW_LIMIT:
    raw_len = len(dataset)
    sampled_groups = []
    for _, group in dataset.groupby('category', sort=False):
        n_keep = min(len(group), max(2, int(ROW_LIMIT * len(group) / raw_len)))
        sampled_groups.append(group.sample(n_keep, random_state=SEED))
    dataset = (
        pd.concat(sampled_groups, ignore_index=True)
        .sample(frac=1.0, random_state=SEED)
        .reset_index(drop=True)
    )
    del sampled_groups; gc.collect()
    print(f"Applied stratified fast local ROW_LIMIT={ROW_LIMIT:,} — shape: {dataset.shape}")
...
LABEL_COL = None
for candidate in ['category', 'subcategory', 'attack']:
    if candidate in dataset.columns:
        LABEL_COL = candidate
        break
if LABEL_COL is None:
    print("❌ No label column found. Expected 'category', 'subcategory', or 'attack'.")
    sys.exit(1)
...
axes[0].set_title('Class Distribution — BoT-IoT', fontweight='bold')
...
print("\n" + "=" * 100)
print("   FINAL COMPARISON — BoT-IoT IDS (Gated Fusion + Train-only WCGAN-GP)")
print("=" * 100)
...
plt.suptitle('Model Comparison — BoT-IoT IDS (Gated Fusion + Train-only WCGAN-GP)',
...
print("\n🎉 BoT-IoT Pipeline complete!")
notify_user("BoT-IoT IDS Complete", f"Pipeline finished. Results saved to {OUTPUT_DIR.resolve()}")
