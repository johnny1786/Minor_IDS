"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   UNSW-NB15 Intrusion Detection System — GATED FUSION LOCAL VERSION        ║
║   Pipeline: Decision Tree + XGBoost + Gated Image-Tabular Fusion + RF/LGBM  ║
║             + Late Fusion | PCA + LDA + train-only WCGAN-GP                 ║
║   Dataset : UNSW-NB15 multiclass CSV (attack_cat target)                    ║
╚══════════════════════════════════════════════════════════════════════════════╝
...
HOW TO RUN:
    ...
    3. python unsw_gated_fusion.py
    4. Results saved to ./outputs_unsw_gated_fusion/
...
DATASET_FILE   = "./data/UNSW_NB15.csv"  # UNSW-NB15 multiclass CSV
OUTPUT_DIR     = "./outputs_unsw_gated_fusion"  # Separate experiment outputs
...
print("=" * 70)
print("  UNSW-NB15 IDS — Local Run (Gated Fusion + Train-only WCGAN-GP)")
print("=" * 70)
...
LABEL_COL = None
for candidate in ['attack_cat', 'type', 'category', 'label', 'attack']:
    if candidate in dataset.columns:
        LABEL_COL = candidate
        break
if LABEL_COL is None:
    print("❌ No label column found. Expected 'attack_cat', 'type', 'category', or 'label'.")
    sys.exit(1)
...
axes[0].set_title('Class Distribution — UNSW-NB15', fontweight='bold')
...
print("\n" + "=" * 100)
print("   FINAL COMPARISON — UNSW-NB15 IDS (Gated Fusion + Train-only WCGAN-GP)")
print("=" * 100)
...
plt.suptitle('Model Comparison — UNSW-NB15 IDS (Gated Fusion + Train-only WCGAN-GP)',
...
notify_user("UNSW-NB15 IDS Complete", f"Pipeline finished. Results saved to {OUTPUT_DIR.resolve()}")
